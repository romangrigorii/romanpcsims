...
side_P = [-4,1,-1, 2, -3, 3, -2, 1,-1, 4]
side_L = [2,1,1,1,1,1,1,1,1,2]
side_T = [-5,-3.5,-2.5,-1.5,-.5,.5,1.5,2.5,3.5,5]
...
base1 = geompy.MakeFaceHW(i_d*12, d_island, 1)
base1 = geompy.MakeTranslation(base1,step_w/2,0,d_glass)
base2 = geompy.MakeFaceHW(i_d*7, d_island, 1)
base2 = geompy.MakeTranslation(base2,step_w/2,0,d_glass)
base3 = geompy.MakeFaceHW(i_d*4, d_island, 1)
base3 = geompy.MakeTranslation(base3,step_w/2,0,d_glass)

IIxlist = [base1];

#for i in range(2):
IIxlist.append(geompy.MakeTranslation(base2,0,i_d,0))
IIxlist.append(geompy.MakeTranslation(base2,0,-i_d,0))
IIxlist.append(geompy.MakeTranslation(base1,0,i_d*2,0))
IIxlist.append(geompy.MakeTranslation(base1,0,-i_d*2,0))
IIxlist.append(geompy.MakeTranslation(base3,0,i_d*3,0))
IIxlist.append(geompy.MakeTranslation(base3,0,-i_d*3,0))

for i in range(2):
    IIxlist.append(geompy.MakeTranslation(base1,0,i_d*2 + i_d*2*(i+1),0))
    IIxlist.append(geompy.MakeTranslation(base1,0,-(i_d*2 + i_d*2*(i+1)),0))
for i in range(10):
    side = geompy.MakeFaceHW(d_island,i_d*side_L[i], 1)
    side = geompy.MakeTranslation(side,step_w/2+i_d*side_P[i],side_T[i]*i_d,d_glass)
    IIxlist.append(side)
...
base1 = geompy.MakeFaceHW(i_d*12 + 2*delet, d_island + 2*delet, 1)
base1 = geompy.MakeTranslation(base1,step_w/2,0,d_glass)
base2 = geompy.MakeFaceHW(i_d*7 + 2*delet, d_island + 2*delet, 1)
base2 = geompy.MakeTranslation(base2,step_w/2,0,d_glass)
base3 = geompy.MakeFaceHW(i_d*4 + 2*delet, d_island + 2*delet, 1)
base3 = geompy.MakeTranslation(base3,step_w/2,0,d_glass)

IIxlist_d = [base1];
delline = geompy.MakeFaceHW(i_d*12 + 2*delet,delet,1)
...
IIxlist_d.append(geompy.MakeTranslation(base2,0,i_d,0))
IIxlist_d.append(geompy.MakeTranslation(base2,0,-i_d,0))
IIxlist_d.append(geompy.MakeTranslation(base1,0,i_d*2,0))
IIxlist_d.append(geompy.MakeTranslation(base1,0,-i_d*2,0))
IIxlist_d.append(geompy.MakeTranslation(base3,0,i_d*3,0))
IIxlist_d.append(geompy.MakeTranslation(base3,0,-i_d*3,0))
IIxlist_d.append(geompy.MakeTranslation(base1,0,i_d*4,0))
IIxlist_d.append(geompy.MakeTranslation(base1,0,-i_d*4,0))
IIxlist_d.append(geompy.MakeTranslation(base1,0,i_d*6,0))
IIxlist_d.append(geompy.MakeTranslation(base1,0,-i_d*6,0))

for i in range(10):
    side = geompy.MakeFaceHW(d_island + 2*delet,i_d*side_L[i] + 2*delet, 1)
    side = geompy.MakeTranslation(side,step_w/2+i_d*side_P[i],side_T[i]*i_d,d_glass)
    IIxlist_d.append(side)

for i in range(inum+1):
    IIxlist_d.append(geompy.MakeTranslation(delline,0,i_d*i,0))
    IIxlist_d.append(geompy.MakeTranslation(delline,0,-i_d*i,0))
...
gen_rx_f = geompy.MakeFaceHW(i_d*8, i_d*4.5, 1)
gen_rx_f = geompy.MakeFaceWires([geompy.MakeEllipse(None, None, i_d*3.5, i_d*3.5)], 1)

gen_rx_t = geompy.MakeFaceHW(step_w, Rx_trunk, 1)
...
gen_Rx_trunkeletion_f = geompy.MakeFaceHW(i_d*8 + delet*2, i_d*4.5 + delet*2, 1)
gen_Rx_trunkeletion_f = geompy.MakeFaceWires([geompy.MakeEllipse(None, None, i_d*3.5 + delet, i_d*3.5 + delet)], 1)

gen_Rx_trunkeletion_t = geompy.MakeFaceHW(step_w, Rx_trunk + delet*2, 1)
gen_Rx_trunkeletion_ft = geompy.MakeFuseList([gen_Rx_trunkeletion_f,gen_Rx_trunkeletion_t])
gen_Rx_trunkeletion_t = geompy.MakeFuseList([geompy.MakeTranslation(gen_Rx_trunkeletion_f,0,step_h,0),geompy.MakeTranslation(gen_Rx_trunkeletion_f,0,-step_h,0),gen_Rx_trunkeletion_t])

gen_cell_Tx = geompy.MakeFaceHW(cell_w_Tx, step_h, 1)

gen_tx_0 = geompy.MakeCutList(gen_cell_Tx, [gen_Rx_trunkeletion_ft], True)
gen_tx_1 = geompy.MakeCutList(gen_cell_Tx, [gen_Rx_trunkeletion_t], True)
...
for i in range(len(faces1)):
    ...
    if (np.abs(xmi-xma)< d_island*1.05) and (np.abs(xmi-xma) > d_island*.95):
            ...
            ixi+=1
    if (np.abs(xmi - xma) < 12.05*i_d) and (np.abs(xmi - xma) > 11.95*i_d):
...
face2_indexes_TxIx = [12,13,14,15,33,34,35,36];
# face2_names_Tx = [];
# face2_indexes_Tx = [0,1,2,3,4,5, 10,13,15,17,19,22, 27,28,29,30,31,32];
face2_indexes_Tx = [1,2,3,5,6,7,8,9,10,11, 16,17,19,22,26,29,31,32, 37,38,39,40,41,42,43,45,46,47];
# face2_names_Rx = [];
...
#         txi+=1
#     if (np.abs(xmi - xma) <  cell_w_Hx*3*1.05) and (np.abs(xmi - xma) > cell_w_Hx*3*.95) and (np.abs(ymi - yma) <  cell_w_Hx*i_d*3.6) and (np.abs(ymi - yma) > cell_w_Hx*i_d*3.4):
