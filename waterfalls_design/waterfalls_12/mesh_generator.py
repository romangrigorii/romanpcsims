...
basei = geompy.MakeFaceHW(cell_w_Hx - d_island*6, d_island, 1)
base = geompy.MakeTranslation(basei,0,0,d_glass)

sidei = geompy.MakeFaceHW(d_island,i_l*2 - d_island*8, 1)
sidei = geompy.MakeTranslation(sidei,0,0,d_glass)
side = geompy.MakeRotation(sidei, OZ, -10.5*math.pi/180.0)

IIxlist = [base,side];

for i in range(nlines-1):
    branch = geompy.MakeFaceHW(cell_w_Hx*(nlines - i)/(nlines+2.5),d_island,1)
    IIxlist.append(geompy.MakeTranslation(branch,cell_w_Hx*(i+1.5)/(nlines+2.5)/2,(i+1)*i_d,d_glass))
    IIxlist.append(geompy.MakeTranslation(branch,-cell_w_Hx*(i+1.5)/(nlines+2.5)/2,-(i+1)*i_d,d_glass))
...
basei = geompy.MakeFaceHW(cell_w_Hx - d_island*6 + delet*2, d_island + 2*delet, 1)
base = geompy.MakeTranslation(basei,0,0,d_glass)

sidei = geompy.MakeFaceHW(d_island + 2*delet, i_l*2 - d_island*8 + 2*delet, 1)
sidei = geompy.MakeTranslation(sidei,0,0,d_glass)
side = geompy.MakeRotation(sidei, OZ, -10.5*math.pi/180.0)
...
for i in range(nlines-1):
    branch = geompy.MakeFaceHW(cell_w_Hx*(nlines - i)/(nlines+2.5) + 2*delet,d_island + 2*delet,1)
    IIxlist_d.append(geompy.MakeTranslation(branch,cell_w_Hx*(i+1.5)/(nlines+2.5)/2,(i+1)*i_d,d_glass))
    IIxlist_d.append(geompy.MakeTranslation(branch,-cell_w_Hx*(i+1.5)/(nlines+2.5)/2,-(i+1)*i_d,d_glass))
...
face_gen = geompy.MakeFaceWires([geompy.MakeEllipse(None, None, Rx_r*2, Rx_r*1)], 1)
gen_rx_f = geompy.MakeRotation(face_gen, OZ, 77.5*math.pi/180.0)
...
face_gen = geompy.MakeFaceWires([geompy.MakeEllipse(None, None, Rx_r*2 + delet, Rx_r*1 + delet)], 1)
gen_rx_deletion_f = geompy.MakeRotation(face_gen, OZ, 77.5*math.pi/180.0)
...
face1_names_Hx = [];
face1_indexes_Hx = [3,9,13,21,25,31];
face1_names_Ix = [];
face1_indexes_Ix = [11,23];
face1_names_IIx = [];
face1_indexes_IIx = [2,5,7,15,17,19,27,29,32]
...
face2_names_Tx = [];
face2_indexes_Tx = [0,2,3,6,7,8,10,11,12,13,16,17,19,20,23,24,25,26,28,29,30,33,34,36];
face2_names_Rx = [];
face2_indexes_Rx = [5,9,18,27,31];
...
for i in range(len(face2_names_Rx)):
