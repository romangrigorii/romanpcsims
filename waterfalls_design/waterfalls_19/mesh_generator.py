...
base = geompy.MakeFaceHW(cell_w_Hx*.9, d_island, 1)
base = geompy.MakeTranslation(base,step_w/2,0,d_glass)
side = geompy.MakeFaceHW(d_island, (inum-.2)*i_d*2, 1)
...
base = geompy.MakeFaceHW(cell_w_Hx*.9 + 2*delet, d_island + 2*delet, 1)
base = geompy.MakeTranslation(base,step_w/2,0,d_glass)
side = geompy.MakeFaceHW(d_island + 2*delet, (inum-.2)*i_d*2 + 2*delet, 1)
side = geompy.MakeTranslation(side,step_w/2,0,d_glass)
IIxlist_d = [base,side];
delline = geompy.MakeFaceHW(cell_w_Hx*.9 + 2*delet,delet,1)
...
for i in range(len(faces1)):
    ...
    if (np.abs(xmi-xma)< d_island*1.05) and (np.abs(xmi-xma) > d_island*.95):
            ...
            ixi+=1
    if (np.abs(xmi - xma) < cell_w_Hx*.9 + delet) and (np.abs(xmi - xma) > cell_w_Hx*.9 - delet):
