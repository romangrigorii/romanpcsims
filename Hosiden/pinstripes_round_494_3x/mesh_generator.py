...
base = geompy.MakeFaceHW(d_island, 3*step_h - d_island*10, 1)
...
Hxlist_i = [];
IIxlist_d = [];
base = geompy.MakeFaceHW(d_island + 2*delet, 3*step_h - d_island*10 + 2*delet, 1)
base = geompy.MakeTranslation(base,0,step_h*3/2,d_glass)
for i in range(inum):
    base = geompy.MakeTranslation(base, i_d, 0, 0)
...
gen_rx_f = geompy.MakeFaceWires([geompy.MakeEllipse(None, None, i_d*5, i_d*3)], 1)
...
gen_Rx_trunkeletion_f = geompy.MakeFaceWires([geompy.MakeEllipse(None, None, i_d*5 + delet, i_d*3 + delet)], 1)
...
Tx2_0 = geompy.MakeTranslation(gen_tx_1,step_w*5/2, step_h/2, 0)
