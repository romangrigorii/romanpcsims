...
distances = [.03,.05,.07,.09,.11,.13,.15]
...
for d in distances:
    ...
    start2 = geompy.MakeFaceHW(d/3, cap_l, 1)
    island = geompy.MakeTranslation(start2,cap_w + d/2,cap_l/2,d_glass)
    ...
    list_of_bodies = [left] + [right] + [island] + [box_OCA,box_PVB,box_air,box_glass,box_spine]
    ...
    geompy.addToStudy( island, 'island' )
    ...
    face1_names = ["left","island","right"]
    face1_indexes = [0,2,4]
    geompy.addToStudyInFather(face_group1,faces1[0],"left")
    geompy.addToStudyInFather(face_group1,faces1[2],"island")
    geompy.addToStudyInFather(face_group1,faces1[4],"right")
    ...
    Mesh_1.ExportUNV(r'/media/sf_Shared_Folder/sim_files_cap2/UNV/Mesh_1_' + f"{int(d*1000):04}" + '.unv')
