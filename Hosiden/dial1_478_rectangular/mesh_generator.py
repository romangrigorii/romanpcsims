...
side_L = [2,2,1,1,1,1,2,2]
side_T = [-5,-3,-1.5,-.5,.5,1.5,3,5]
...
base2 = geompy.MakeFaceHW(i_d*8, d_island, 1)
base2 = geompy.MakeTranslation(base2,step_w/2,0,d_glass)

IIxlist = [base2];

for i in range(2):
    IIxlist.append(geompy.MakeTranslation(base2,0,i_d*(i+1),0))
    IIxlist.append(geompy.MakeTranslation(base2,0,-i_d*(i+1),0))
for i in range(2):
...
base1 = geompy.MakeTranslation(base1,step_w/2,0,d_glass)
base2 = geompy.MakeFaceHW(i_d*8 + 2*delet, d_island + 2*delet, 1)
base2 = geompy.MakeTranslation(base2,step_w/2,0,d_glass)
...
for i in range(2):
    IIxlist_d.append(geompy.MakeTranslation(base2,0,i_d*(i+1),0))
    IIxlist_d.append(geompy.MakeTranslation(base2,0,-i_d*(i+1),0))
for i in range(2):
    IIxlist_d.append(geompy.MakeTranslation(base1,0,i_d*2 + i_d*2*(i+1),0))
    IIxlist_d.append(geompy.MakeTranslation(base1,0,-(i_d*2 + i_d*2*(i+1)),0))
for i in range(8):
