...
step = 2.695
cell_w_Hx = step - Hx_delet*2 - d_island
...
Hxtype = 0; # 0: resistive 1: grid 2: snake
Hxthick = .1;
Hxdel = .1;
...
gen_cell_Hx = geompy.MakeFaceHW(cell_w_Hx, 2*step*ver, 1)

Hx0t = geompy.MakeTranslation(gen_cell_Hx, step/2, ver*step, d_glass)
Hx0 = Hx0t
if Hxtype == 1:
...
gen_i = geompy.MakeFaceHW(d_island, 2*step*ver, 1)
Ix = geompy.MakeTranslation(gen_i,step, step*ver, d_glass)
Ixlist = [Ix];
...
gen_cell_Tx = geompy.MakeFaceHW(cell_w_Tx, step, 1)
...
box_air0 = geompy.MakeBoxDXDYDZ(step*2*hor,step*ver*2,d_air+d_glass+d_PVB+d_spine+d_OCA)
box_air1 = geompy.MakeTranslation(box_air0,0,0, -(d_air + d_glass + d_PVB + d_spine + d_OCA)/2 - ((d_PVB + d_spine + d_OCA - d_glass)/2))

box_glass0 = geompy.MakeBoxDXDYDZ(step*2*hor,step*ver*2,d_glass)
box_glass = geompy.MakeTranslation(box_glass0,0,0,0)

box_PVB0 = geompy.MakeBoxDXDYDZ(step*2*hor,step*ver*2,d_PVB)
box_PVB = geompy.MakeTranslation(box_PVB0,0,0,-d_PVB)

box_spine0 = geompy.MakeBoxDXDYDZ(step*2*hor,step*ver*2,d_spine)
box_spine = geompy.MakeTranslation(box_spine0,0,0,-d_PVB - d_spine)

box_OCA0 = geompy.MakeBoxDXDYDZ(step*2*hor,step*ver*2,d_OCA)
...
face1_names_Hx = [];
face1_indexes_Hx = [1,5];
face1_names_Ix = [];
face1_indexes_Ix = [3];

for i in range(hor*2):
    face1_names_Hx.append('Hx' + str(i))
for i in range((hor-1)*2):
    face1_indexes_Hx.append(face1_indexes_Hx[-1]+4)

for i in range(hor*2 - 1):
    face1_names_Ix.append('Ix' + str(i))

for i in range((hor-1)*2):
    face1_indexes_Ix.append(face1_indexes_Ix[-1]+4)

face1_names = face1_names_Hx + face1_names_Ix
face1_indexes = face1_indexes_Hx + face1_indexes_Ix
faces1_new = [];
