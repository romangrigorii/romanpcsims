...
base1 = geompy.MakeFaceHW(i_d*11, d_island, 1)
base1 = geompy.MakeTranslation(base1,step_w/2,0,d_glass)
base2 = geompy.MakeFaceHW(i_d*11, d_island, 1)
base2 = geompy.MakeTranslation(base2,step_w/2,0,d_glass)
IIxlist = [base2];

for i in range(2):
    IIxlist.append(geompy.MakeTranslation(base1,0,i_d*(i+1),0))
    IIxlist.append(geompy.MakeTranslation(base1,0,-i_d*(i+1),0))
for i in range(2):
...
base = geompy.MakeFaceHW(i_d*11 + 2*delet, d_island + 2*delet, 1)
base = geompy.MakeTranslation(base,step_w/2,0,d_glass)
IIxlist_d = [base];

delline = geompy.MakeFaceHW(i_d*11 + 2*delet,delet,1)
...
gen_rx_f = geompy.MakeFaceHW(i_d*11, i_d*4.5, 1)
...
gen_Rx_trunkeletion_f = geompy.MakeFaceHW(i_d*11 + delet*2, i_d*4.5 + delet*2, 1)
...
for i in range(len(faces1)):
    ...
    if (np.abs(xmi-xma)< d_island*1.05) and (np.abs(xmi-xma) > d_island*.95):
            ...
            ixi+=1
    if (np.abs(xmi - xma) < (11*i_d + delet/2)) and (np.abs(xmi - xma) >= (11*i_d - delet/2)):
