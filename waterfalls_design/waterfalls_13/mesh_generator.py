...
Rx_d = 0.2
lines_per_cell = 3
nlines = lines_per_cell*3 + 2

step_w = 3.5933333
step_h = 2.695

i_l = step_h*3 - d_island*3
i_d = step_h/lines_per_cell
...
base = geompy.MakeFaceHW(cell_w_Hx/(nlines)*(nlines+1), d_island, 1)
basem = geompy.MakeTranslation(base,0,0,d_glass)

side = geompy.MakeFaceHW(d_island,i_d*(nlines - 4), 1)
sider = geompy.MakeTranslation(side,cell_w_Hx*1/3 - delet,i_d*(nlines - 3)/2,d_glass)
side = geompy.MakeFaceHW(d_island,i_d*(nlines - 3), 1)
sidel = geompy.MakeTranslation(side,-cell_w_Hx*1/3 + delet,-i_d*(nlines - 4)/2,d_glass)

IIxlist = [sidel,sider];

for i in range(nlines - 3):
    if i == 0:
        branch = geompy.MakeFaceHW(cell_w_Hx*(nlines)/(nlines + 3), d_island ,1)
        IIxlist.append(geompy.MakeTranslation(branch,0,(i+.5)*i_d,d_glass))
        IIxlist.append(geompy.MakeTranslation(branch,0,-(i+.5)*i_d,d_glass))
    else:
        branch = geompy.MakeFaceHW(cell_w_Hx*(nlines - i - 2)/(nlines + 3), d_island ,1)
        IIxlist.append(geompy.MakeTranslation(branch,cell_w_Hx*(i+2)/(nlines + 3)/2,(i+.5)*i_d,d_glass))
        IIxlist.append(geompy.MakeTranslation(branch,-cell_w_Hx*(i+2)/(nlines + 3)/2,-(i+.5)*i_d,d_glass))

IIx = geompy.MakeFuseList(IIxlist, True, True)
IIx = geompy.MakeTranslation(IIx,step_w/2,-step_h*5/2,0)

IIxlist = [IIx]

for i in range(2):
    IIxlist.append(geompy.MakeTranslation(IIxlist[-1],0,step_h*3,0))
...
side = geompy.MakeFaceHW(d_island + 2*delet,i_d*(nlines - 4) + 2*delet, 1)
sider = geompy.MakeTranslation(side,cell_w_Hx*1/3 - delet,i_d*(nlines-3)/2,d_glass)
side = geompy.MakeFaceHW(d_island + 2*delet,i_d*(nlines - 3) + 2*delet, 1)
sidel = geompy.MakeTranslation(side,-cell_w_Hx*1/3 + delet,-i_d*(nlines - 4)/2,d_glass)
...
for i in range(nlines - 3):
    if i == 0:
        branch = geompy.MakeFaceHW(cell_w_Hx*(nlines)/(nlines + 3) + 2*delet,d_island + 2*delet,1)
        IIxlist_d.append(geompy.MakeTranslation(branch,0,(i+.5)*i_d,d_glass))
        IIxlist_d.append(geompy.MakeTranslation(branch,0,-(i+.5)*i_d,d_glass))
    else:
        branch = geompy.MakeFaceHW(cell_w_Hx*(nlines - i - 2)/(nlines + 3) + 2*delet,d_island + 2*delet,1)
        IIxlist_d.append(geompy.MakeTranslation(branch,cell_w_Hx*(i+2)/(nlines + 3)/2,(i+.5)*i_d,d_glass))
        IIxlist_d.append(geompy.MakeTranslation(branch,-cell_w_Hx*(i+2)/(nlines + 3)/2,-(i+.5)*i_d,d_glass))

IIx_d = geompy.MakeFuseList(IIxlist_d, True, True)
IIx_d = geompy.MakeTranslation(IIx_d,step_w/2,-step_h*5/2,0)

IIxlist_d = [IIx_d]

for i in range(2):
    IIxlist_d.append(geompy.MakeTranslation(IIxlist_d[-1],0,step_h*3,0))
...
face_gen = geompy.MakeFaceWires([geompy.MakeEllipse(None, None, Rx_r*2.1, Rx_r*1.05)], 1)
gen_rx_f = geompy.MakeRotation(face_gen, OZ, 67.5*math.pi/180.0)
...
Rx0_2 = geompy.MakeTranslation(gen_rx_t,step_w/2, step_h*5/2, 0)
Rx0_3 = geompy.MakeTranslation(gen_rx_ft,step_w/2, step_h*7/2, 0)

Rx1_0 = geompy.MakeTranslation(gen_rx_t,step_w*3/2, step_h/2, 0)
Rx1_1 = geompy.MakeTranslation(gen_rx_ft,step_w*3/2, step_h*3/2, 0)
Rx1_2 = geompy.MakeTranslation(gen_rx_t,step_w*3/2, step_h*5/2, 0)

Rx2_0 = geompy.MakeTranslation(gen_rx_t,step_w*5/2, step_h/2, 0)
...
Rx2_3 = geompy.MakeTranslation(gen_rx_ft,step_w*5/2, step_h*-1/2, 0)

Rx0 = geompy.MakeFuseList([Rx0_0, Rx1_0, Rx2_0, Rx0_3], True, True)
Rx1 = geompy.MakeFuseList([Rx0_1, Rx1_1, Rx2_1], True, True)
Rx2 = geompy.MakeFuseList([Rx0_2, Rx1_2, Rx2_2, Rx2_3], True, True)
...
face_gen = geompy.MakeFaceWires([geompy.MakeEllipse(None, None, Rx_r*2.1 + delet, Rx_r*1.05 + delet)], 1)
gen_rx_deletion_f = geompy.MakeRotation(face_gen, OZ, 67.5*math.pi/180.0)
...
gen_tx_b = geompy.MakeCutList(gen_cell_Tx, [gen_rx_deletion_ft_bot,gen_rx_deletion_t], True)
gen_tx_t = geompy.MakeCutList(gen_cell_Tx, [gen_rx_deletion_ft_top,gen_rx_deletion_t], True)

Tx0_0 = geompy.MakeTranslation(gen_tx_0,step_w/2, step_h/2, 0)
Tx0_1 = geompy.MakeTranslation(gen_tx_t,step_w/2, step_h*3/2, 0)
Tx0_2 = geompy.MakeTranslation(gen_tx_b,step_w/2, step_h*5/2, 0)
Tx1_0 = geompy.MakeTranslation(gen_tx_b,step_w*3/2, step_h/2, 0)
Tx1_1 = geompy.MakeTranslation(gen_tx_0,step_w*3/2, step_h*3/2, 0)
Tx1_2 = geompy.MakeTranslation(gen_tx_t,step_w*3/2, step_h*5/2, 0)
Tx2_0 = geompy.MakeTranslation(gen_tx_t,step_w*5/2, step_h/2, 0)
Tx2_1 = geompy.MakeTranslation(gen_tx_b,step_w*5/2, step_h*3/2, 0)
...
face1_names_Hx = [];
face1_indexes_Hx = [3,7, 15,19, 25,31];
face1_names_Ix = [];
face1_indexes_Ix = [11, 23];
face1_names_IIx = [];
face1_indexes_IIx = [1,5,8, 13,17,21,  27,29,33];
...
face2_names_Tx = [];
face2_indexes_Tx = [0,2,3,6,7,9,10,11, 12,13,16,17,19,20,23,24, 25,26,27,29,30,33,34,36];
face2_names_Rx = [];
face2_indexes_Rx = [5,8 ,18, 28,31];
