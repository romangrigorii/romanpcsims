...
base = geompy.MakeTranslation(base,-i_d/2,step_h*3/2,d_glass)
IIxlist = [];
for i in range(inum):
...
gen_rx_f = geompy.MakeFaceWires([geompy.MakeEllipse(None, None, i_d*4.8, i_d*2.8)], 1)
...
gen_Rx_trunkdeletion_f = geompy.MakeFaceWires([geompy.MakeEllipse(None, None, i_d*4.8 + delet, i_d*2.8 + delet)], 1)
gen_Rx_trunkdeletion_t = geompy.MakeFaceHW(step_w, Rx_trunk + delet*2, 1)
gen_Rx_trunkdeletion_ft = geompy.MakeFuseList([gen_Rx_trunkdeletion_f,gen_Rx_trunkdeletion_t])

gen_Rx_trunkdeletion_t1 = geompy.MakeFuseList([geompy.MakeTranslation(gen_Rx_trunkdeletion_f,0, step_h,0),gen_Rx_trunkdeletion_t])
gen_Rx_trunkdeletion_t2 = geompy.MakeFuseList([geompy.MakeTranslation(gen_Rx_trunkdeletion_f,0,-step_h,0),gen_Rx_trunkdeletion_t])

gen_cell_Tx = geompy.MakeFaceHW(cell_w_Tx, step_h, 1)

gen_tx_0 = geompy.MakeCutList(gen_cell_Tx, [gen_Rx_trunkdeletion_ft], True)
gen_tx_1 = geompy.MakeCutList(gen_cell_Tx, [gen_Rx_trunkdeletion_t1], True)
gen_tx_2 = geompy.MakeCutList(gen_cell_Tx, [gen_Rx_trunkdeletion_t2], True)

Tx0_0 = geompy.MakeTranslation(gen_tx_0,step_w/2, step_h/2, 0)
Tx0_1 = geompy.MakeTranslation(gen_tx_2,step_w/2, step_h*3/2, 0)
Tx0_2 = geompy.MakeTranslation(gen_tx_1,step_w/2, step_h*5/2, 0)
Tx1_0 = geompy.MakeTranslation(gen_tx_1,step_w*3/2, step_h/2, 0)
Tx1_1 = geompy.MakeTranslation(gen_tx_0,step_w*3/2, step_h*3/2, 0)
Tx1_2 = geompy.MakeTranslation(gen_tx_2,step_w*3/2, step_h*5/2, 0)
Tx2_0 = geompy.MakeTranslation(gen_tx_2,step_w*5/2, step_h/2, 0)
...
face2_names_TxIx = [];
face2_indexes_TxIx = [11,12,13,14, 28,29,30,31];
face2_names_Tx = [];
face2_indexes_Tx = [1,2,3,4,5,6,9,10, 15,16,18,20,22,24,26,27, 32,33,34,36,37,38,39,40,41];
face2_names_Rx = [];
face2_indexes_Rx = [8,21,34];
...
#         txi+=1
#     if (np.abs(xmi - xma) <  cell_w_Hx*3*1.05) and (np.abs(xmi - xma) > cell_w_Hx*3*.95) and (np.abs(ymi - yma) <=  i_d*3.01) and (np.abs(ymi - yma) >= i_d*2.99):
