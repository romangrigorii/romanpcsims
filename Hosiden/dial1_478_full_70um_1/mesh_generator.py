...
base1 = geompy.MakeFaceHW(i_d*7, d_island, 1)
base1 = geompy.MakeTranslation(base1,step_w/2,0,d_glass)
IIxlist = []

for i in range(2):
    IIxlist.append(geompy.MakeTranslation(base1,0,i_d*(i+.5),0))
    IIxlist.append(geompy.MakeTranslation(base1,0,-i_d*(i+.5),0))
for i in range(1):
    IIxlist.append(geompy.MakeTranslation(base1,0,i_d*3 + i_d*(i+.5),0))
    IIxlist.append(geompy.MakeTranslation(base1,0,-(i_d*3 + i_d*(i+.5)),0))
for i in range(5):
...
base = geompy.MakeFaceHW(i_d*7 + 2*delet, d_island + 2*delet, 1)
base = geompy.MakeTranslation(base,step_w/2,0,d_glass)
IIxlist_d = [];

delline = geompy.MakeFaceHW(i_d*7 + 2*delet,delet,1)
...
for i in range(2):
    IIxlist_d.append(geompy.MakeTranslation(base,0,i_d*(i+.5),0))
    IIxlist_d.append(geompy.MakeTranslation(base,0,-i_d*(i+.5),0))
for i in range(1):
    IIxlist_d.append(geompy.MakeTranslation(base,0,i_d*3 + i_d*(i+.5),0))
    IIxlist_d.append(geompy.MakeTranslation(base,0,-(i_d*3 + i_d*(i+.5)),0))
...
IIxlist_d.append(geompy.MakeTranslation(delline,0,i_d*2.5,0))
IIxlist_d.append(geompy.MakeTranslation(delline,0,-i_d*2.5,0))
...
gen_rx_f = geompy.MakeFaceHW(i_d*7, i_d*3.25, 1)
...
gen_Rx_trunkeletion_f = geompy.MakeFaceHW(i_d*7 + delet*2, i_d*3.25 + delet*2, 1)
...
for i in range(len(faces1)):
    ...
    if (np.abs(xmi-xma)< d_island_i*1.05) and (np.abs(xmi-xma) > d_island_i*.95):
            ...
            ixi+=1
    if (np.abs(xmi - xma) < (7*i_d + delet/2)) and (np.abs(xmi - xma) >= (7*i_d - delet/2)):
