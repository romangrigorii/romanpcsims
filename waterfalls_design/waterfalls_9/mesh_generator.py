...
gen_i = geompy.MakeFaceHW(d_island, step_h*3 - d_island*2 - 2*delet, 1)
IIxlist = [geompy.MakeTranslation(gen_i,i_d, -step_h*(.6 + 3), d_glass)]
for i in range(inum-1):
    IIxlist.append(geompy.MakeTranslation(IIxlist[-1], i_d, step_h*3/inum, 0))

for v in range((ver - 1)*3 + 2):
    for i in range(inum):
        IIxlist.append(geompy.MakeTranslation(IIxlist[-inum],0,step_h*3,0))

for h in range((hor - 1)*3 + 2):
    for i in range(ver*inum*3):
            if np.mod(h,2)==0:
                IIxlist.append(geompy.MakeTranslation(IIxlist[-ver*inum*3],step_w,1.0*step_h,0))
            else:
                IIxlist.append(geompy.MakeTranslation(IIxlist[-ver*inum*3],step_w,1.0*step_h,0))
...
gen_i_del = geompy.MakeFaceHW(d_island + 2*delet, step_h*3 - d_island*2, 1)
IIxlist_d = [geompy.MakeTranslation(gen_i_del ,i_d, -step_h*(.6 + 3), d_glass)]
for i in range(inum-1):
    IIxlist_d.append(geompy.MakeTranslation(IIxlist_d[-1], i_d, step_h*3/inum, 0))

Hxlist_i.append(geompy.MakeCutList(Hxlist[0], IIxlist_d[-inum:], True))

for v in range((ver-1)*3 + 2):
    for i in range(inum):
        IIxlist_d.append(geompy.MakeTranslation(IIxlist_d[-inum],0,step_h*3,0))
    Hxlist_i[0] = geompy.MakeCutList(Hxlist_i[0], IIxlist_d[-inum:], True)

for h in range((hor - 1)*3 + 2):
    for i in range(ver*inum*3):
        if np.mod(h,2)==0:
            IIxlist_d.append(geompy.MakeTranslation(IIxlist_d[-ver*inum*3],step_w,1.0*step_h,0))
        else:
            IIxlist_d.append(geompy.MakeTranslation(IIxlist_d[-ver*inum*3],step_w,1.0*step_h,0))

    Hxlist_i.append(geompy.MakeCutList(Hxlist[h+1], IIxlist_d[-ver*inum*3:], True))
...
face_gen = geompy.MakeFaceWires([geompy.MakeEllipse(None, None, Rx_r*2.0, Rx_r*1)], 1)
gen_rx_f = geompy.MakeRotation(face_gen, OZ, 80*math.pi/180.0)
gen_rx_t = geompy.MakeFaceHW(step_w, Rx_d, 1)
gen_rx_ft = geompy.MakeFuseList([gen_rx_f, gen_rx_t], True, True)
...
face_gen = geompy.MakeFaceWires([geompy.MakeEllipse(None, None, Rx_r*2.0 + delet, Rx_r*1 + delet)], 1)
gen_rx_deletion_f = geompy.MakeRotation(face_gen, OZ, 80*math.pi/180.0)
...
for h in range(ver*3*inum*hor*3):
...
face1_names_Hx = [];
face1_indexes_Hx = [9,29,49]; # 8 islands [15,53,91]
face1_names_Ix = [];
face1_indexes_Ix = [19,39]; # 8 islands [35,71]
face1_names_IIx = [];
face1_indexes_IIx = [1,4,5,8,10,13,14,17, 21,24,25,28,30,33,34,37,  41,44,45,48,50,53,54,57]; #[1,4,5,8,9,12,13,17,18,21,22,24,27,29,33,36,37,40,41,44,45,48,51,53,54,57,58,61,65,67,70,72,73,76,77,81,82,85,86,89,90,93]; # 8 islands [1,4,5,8,9,12,13,17,18,21,22,25,26,28,31,33,37,40,41,44,45,48,49,51,55,57,58,61,62,65,66,69,73,75,78,80,81,84,85,88,89,93,94,97,98,101,102,105];
...
face2_names_Tx = [];
face2_indexes_Tx = [0,2,3,4,7,8,10,11, 12,13,16,17,19,20,23,24, 25,26,28,29,32,33,34,36];
face2_names_Rx = [];
face2_indexes_Rx = [6,9 ,18, 27,30];
